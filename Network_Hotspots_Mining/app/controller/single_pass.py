...
class SinglePassCluster():
    def __init__(self, stopWords_path="app/data/stop_words.txt", my_stopwords=None,
                 ...
                 simi_threshold=0.5, res_save_path1="app/result/res_cluster2idx.json", res_save_path2="app/result/res_total.json"):
    ...
    def single_pass(self, texts):
        ...
        for idx, vec in enumerate(tfidf):
            ...
            if not self.cluster_center_vec:
                ...
                self.res[0] = {idx: self.idx_2_text[idx]}
            # Clusters exist
            else:
                max_simi, max_idx = self.cosion_simi(vec)
                if max_simi >= self.simi_thr:
                    self.cluster_2_idx[max_idx].append(idx)
                    self.res[max_idx][idx] = self.idx_2_text[idx]
                else:
                    new_cluster_id = len(self.cluster_2_idx)
                    self.cluster_center_vec.append(vec)
                    self.cluster_2_idx[new_cluster_id] = [idx]
                    self.res[new_cluster_id] = {idx: self.idx_2_text[idx]}
